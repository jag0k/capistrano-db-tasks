  ...
  private
    ...
    def import_cmd(file)
      if mysql?
        ...
        terminate_connection_sql = "SELECT pg_terminate_backend(pg_stat_activity.procpid) FROM pg_stat_activity WHERE pg_stat_activity.datname = '#{database}' AND pid <> pg_backend_pid();"
