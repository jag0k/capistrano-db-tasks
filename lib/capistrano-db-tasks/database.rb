  ...
  private
    ...
    def dump_cmd
      if mysql?
        ...
        "#{pgpass} pg_dump --no-acl --no-owner #{credentials} #{database} | grep -v lock_timeout"
      end
    end
    ...
    def import_cmd(file)
      if mysql?
        ...
        terminate_connection_sql = "SELECT pg_terminate_backend(pg_stat_activity.procpid) FROM pg_stat_activity WHERE pg_stat_activity.datname = '#{database}' AND procpid <> pg_backend_pid();"
